...
# Update this depending on your local setup
file_path <-  "D:\\Northeastern\\"

# We won't be using the Kaggle training or test data. We'll create our own labels
#train <- as.tibble(fread(paste0(file_path,'train_v2.csv')))
#test <- as.tibble(fread('sample_submission_v2.csv'))
members <- as.tibble(fread(paste0(file_path,'members_v3.csv'), nrows = 1e6))
trans <- as.tibble(fread(paste0(file_path,'transactions.csv')))
trans2 <- as.tibble(fread(paste0(file_path,'transactions_v2.csv')))
trans_combined <- bind_rows(trans, trans2)

logs <- as.tibble(fread(paste0(file_path,'user_logs_v2.csv'), nrows = 5e6))
```
